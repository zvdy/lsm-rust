...
impl CompactionManager {
    ...
    pub fn should_compact(&self, level: usize, tables: &[SSTable]) -> bool {
        ...
        let level_threshold = self.size_threshold * (self.level_multiplier as usize).pow(level as u32);
        println!("Level {} size: {} bytes, threshold: {} bytes", level, level_size, level_threshold);
    ...
    pub fn compact(&self, tables: &[SSTable]) -> io::Result<SSTable> {
        ...
        let mut new_table = SSTable::new(tables[0].get_path().with_file_name(format!("compact_{}.sst", 
